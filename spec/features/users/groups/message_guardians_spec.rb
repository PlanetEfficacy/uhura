...
RSpec.feature "user messages a group of students' guardians" do
  scenario "from the new group message form" do
    ...
    click_link "Guardians"

    expect(current_path).to eq(new_group_guardian_message_path(group))

    within "h4.contact-header" do
      expect(page).to have_content("Message to guardians of group: #{group.name}")
    end
